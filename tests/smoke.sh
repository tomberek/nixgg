...
#
# tests/drv-equivalence.sh only compares drv hashes; it never realises
# an output, so it once reported a clean 149/149 while link outputs
# had silently moved and native builds failed to collect their
# artifact. It also can't cover llvm/two-phase, which have no single
# `src` to resolve natively (multi-phase, one source per phase).
#
...
#
# The expected path encodes the FHS layout (link -> bin/, ar -> lib/)
# so a change to output placement that forgets a consumer fails here
# loudly instead of in a user's terminal.
#
# "-" as the run command means the artifact is a library: existing at
# the right path with non-zero size is all we assert.
QUICK=(
  "hello|bin/hello|%s"
  "two-phase|bin/app|%s"
  "fmt|lib/libfmt.a|-"
...
SLOW=(
  ...
  # nix-full's default target is libnixutil, a library, so the
  # runnable target is addressed explicitly via .packages.nix.
...
# predicted $ALT_STORE path: these binaries' baked RPATH/RUNPATH
# entries point at real (non-fixed-output) sibling store paths — e.g.
# zstd-dyndrv's bin/zstd needs its own "out" output's libzstd.so.1 —
# that exist only inside $ALT_STORE, not at the real filesystem root.
# The dynamic loader always resolves an absolute RPATH against the
# real root, so a direct exec fails with "cannot open shared object
# file" even on a correct build; only `nix run`'s private mount
# namespace (bind-mounting $ALT_STORE onto /nix/store) resolves it.
# QUICK/SLOW's direct-exec check never caught this class of bug
# because their only runtime dep is glibc, which happens to already
# exist at the same path outside the alt store too (fixed-output
# derivation).
DYNDRV=(
...
# existing nixpkgs package" story as DYNDRV, but split at the
# configure/build boundary instead of build/install, with no
# sandbox/shims/RPC at all. Same nix-run-only rationale as DYNDRV.
# hello-cache-filtered and fmt-cache-filtered additionally exercise
# configureSrcFilter's early-cutoff — this smoke test only covers
# "does it still build and run", not the caching behavior itself (see
# tests/configure-cache-cutoff.sh for that).
CONFIGCACHE=(
...
# two. Same nix-run-only rationale as DYNDRV.
DYNCONFIGCACHE=(
...
# direct_exec=1: also exec $ALT_STORE's on-disk path directly.
# direct_exec=0 (DYNDRV/CONFIGCACHE/DYNCONFIGCACHE): skip straight to
# `nix run`/`nix shell` — a direct exec is expected to fail there
# regardless of build correctness (see those sets' own comments above).
check_example() {
  local attr="$1" want="$2" run="$3" direct_exec="$4"
  printf '\033[1;36m===== %s =====\033[0m\n' "$attr"

  # mainProgram must be set iff the artifact is meant to be run: a
  # library incorrectly claiming one would send `nix run` at a
  # nonsense path, and a binary missing one degrades silently since
  # Nix's pname fallback happens to paper over it for every current
  # example. Skipped for direct_exec=0: those wrap an arbitrary
  ...
  # A multi-output build (e.g. zstd-dyndrv's bin+man) prints one line
  # per realized output, in no guaranteed order — pick whichever
  # printed output actually has the wanted artifact.
  out=""
  ...
  if [[ "$direct_exec" == "1" ]]; then
    ...
    if out_txt=$(eval "$cmd" 2>&1 | head -1); then
      ...
      # reason unrelated to the build: its ELF interpreter/RPATH
      # resolve against the REAL filesystem root, so it only works if
      # a fixed-output dep like glibc ALSO happens to already exist at
      # that same path outside the alt store. Don't hard-fail on it;
      # fall through to the `nix run` check below, which uses Nix's
      # own private mount-namespace bind mount of $ALT_STORE onto
      # /nix/store and is the actually reliable verification path.
  ...
  # the path we predicted. Reuses the build above (same drv, cached),
