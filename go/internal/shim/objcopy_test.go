...
// The modelled shape is `objcopy <flags> <in> <out>`. Only that
// two-operand rewrite is modelled; in-place and read-only forms must
// bail so the real tool runs.
...
// A long flag with its value attached via `=` needs no two-arg
// handling — the exact invocation that first failed with "gdt_idt.o:
// file format not recognized" before objcopy was modelled.
