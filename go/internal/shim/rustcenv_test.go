...
//
// gets a value naming the caller's build tree, which the sandbox
// doesn't have. The same tree is mounted at the staged store path, so
// the value has to name that instead — passed verbatim, rustc fails
// on a missing file and points at the include, not at the variable.
