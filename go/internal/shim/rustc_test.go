...
func TestParseRustcArgsKernelShapes(t *testing.T) {
	t.Run("cmd_rustc_o_rs", func(t *testing.T) {
		...
		// -L and --out-dir describe the build tree, which the sandbox
		// does not have. They must not reach the derivation.
...
// This shim is the only one normally reached through PATH rather than
// an explicit tool= assignment: Kbuild's default is a bare `rustc` and
// nixpkgs doesn't override it. So the PATH fallback must not find the
// shim again — that forks the build until it dies, with no error
// naming the cause.
...
// `if_changed_dep` runs fixdep over it and hard-fails when it's
// missing, and the derivation can't write it — the emit flags were
...
// The real command line Kbuild records for a Rust driver, verbatim
// from drivers/gpu/drm/.drm_panic_qr.o.cmd. Here because reading the
// argv grammar wasn't enough: `--remap-path-prefix FROM=TO` is a
// SEPARATED two-argument flag, its value was taken for a second
// positional, and the parser bailed. That bail is safe — the build
// runs unaccelerated rather than wrong — but it's silent, so it cost a
// full kernel run to notice.
func TestParseRustcArgsRealKernelDriver(t *testing.T) {
	...
	// A two-argument flag whose value was mistaken for the source is
	// the exact shape of the bug this pins, so check the value didn't
	// leak back out as a flag of its own.
	for _, f := range rc.Flags {
...
// kernel crate compiled under `--sysroot=/dev/null` can't resolve
...
func TestParseRustcArgsKeepsScanFlags(t *testing.T) {
	...
	// The derivation's flags stay reduced — -L names the build tree
	// and externs are resolved to explicit paths.
	for _, f := range rc.Flags {
...
// A kernel's generated cfg file is hundreds of KB — one
...
// to model the compile puts all of it into the single `bash -c`
...
// A spec already in the store keeps its path: it's content-addressed
// and mounted already, and re-adding it would change the filename
// rustc derives the target NAME from.
