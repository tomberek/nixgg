...
// The property RunRust depends on: rustc writes the dep-info file even
// when the crate FAILS to compile. Under nixgg a crate's `--extern`
// dependencies are drvref stubs, so every scan is such a failing
// compile — if rustc ever stopped writing dep-info on error, the shim
// would stage an incomplete source tree and the resulting derivation
// would fail deep inside the sandbox, naming a missing module rather
// than the scan.
//
// Also pins that `include!` is reported — the reason this scanner is
// worth having: it closes, for Rust, the hole `gcc -M` leaves for
// `.incbin` (a file the compiler reads at expansion time, named in a
// string no preprocessor examines).
func TestRunRustFindsIncludesDespiteBrokenExtern(t *testing.T) {
	rustc := requireRustc(t)

	root := t.TempDir()
	mustWriteRust(t, filepath.Join(root, "sub", "inc.rs"), "pub const X: u32 = 1;\n")
	mustWriteRust(t, filepath.Join(root, "main.rs"),
	...
	if !hasRustRel(r.Headers, "sub/inc.rs") {
		t.Errorf("include!(\"sub/inc.rs\") missing from scan; got %v", rustRels(r.Headers))
	}
}
...
func TestRunRustLeavesCallerOutputsAlone(t *testing.T) {
	...
	root := t.TempDir()
	mustWriteRust(t, filepath.Join(root, "lib.rs"), "pub fn hi() -> u32 { 7 }\n")
	chdirRust(t, root)
...
func hasRustRel(hs []Header, rel string) bool {
	for _, h := range hs {
		if h.Rel == rel {
			return true
		}
...
func rustRels(hs []Header) []string {
	var out []string
	for _, h := range hs {
...
func mustWriteRust(t *testing.T, path, body string) {
	t.Helper()
...
func chdirRust(t *testing.T, dir string) {
	t.Helper()
	old, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
