...
// be answered by the compiler front end — which rustc answers directly
// via `--emit=dep-info`, listing every file the crate reaches: `mod`
// files, `include!`, `include_str!` and `include_bytes!`. That last
// group matters because it's exactly the case `gcc -M` can't see (the
// `.incbin` problem the compile shim has to pass through for); here it
// comes for free.
//
...
// non-zero — but writes the dep-info file anyway, since dependency
// collection happens during macro expansion, before and independently
// of crate resolution.
//
// The one gap that leaves: a file pulled in by an `include!` that a
// PROC-MACRO generated is invisible, because an unresolvable
// proc-macro crate never expands. That's why shim.Rustc refuses to
...
func runRustScanner(rustc, source string, flags []string) (*Result, []depEntry, error) {
	...
	// rustc writes the crate's real artifacts unless told otherwise,
	// and the caller's --out-dir points into the build tree. Redirect
	// both the dep file and the artifacts into a scratch dir so a scan
	// never touches what the build is about to produce.
	...
	if err != nil {
		// No dep file at all means the front end never got far enough
		// to expand the crate — the source set is unknown, not empty,
		// and staging an empty tree would produce a drv that fails
		// inside the sandbox with an error pointing at the wrong
		// thing.
	...
	for _, tok := range parseMakeDeps(out) {
		...
		if strings.HasPrefix(abs, "/nix/store/") {
			// Already content-addressed; referenced by absolute path
			// and mounted via storeDeps rather than copied into the
			// tree.
			continue
		}
...
// would make the scan write the artifact the build is waiting for,
// from a compile whose externs are stubs.
//
...
// tail returns the last n bytes of s, so a compiler's diagnostic
// reaches the log without a thousand lines of lint output burying it.
